...
for FigNum in range(1,numTest+1):
    b = smoothed(np.load('Avs1_R_mean_0%1s.npy'%(FigNum)))
...
plt.xlabel('Trial')
for FigNum in range(1,numTest+1):
    b = smoothed(np.load('Avs1_gamesT_0%1s.npy'%(FigNum)))
    plt.plot(np.arange(1,N/binSize+1)*binSize, b,colors[FigNum-1])
plt.grid()
...
for FigNum in range(1,numTest+1):
    b = smoothed(np.load('Avs1_R_win_0%1s.npy'%(FigNum)))
    plt.plot(np.arange(1,N/binSize+1)*binSize, b,colors[FigNum-1])
plt.grid()
...
plt.xlabel('Trial')
for FigNum in range(1,numTest+1):
    b = smoothed(np.load('Avs1_transfNum_0%1s.npy'%(FigNum)))
    plt.plot(np.arange(1,N/binSize+1)*binSize, b,'r')#colors[FigNum-1])
    
    bb = smoothed(np.load('Avs1_transfEqual_0%1s.npy'%(FigNum)))
    plt.plot(np.arange(1,N/binSize+1)*binSize, bb,'b')#colors[FigNum-1])
    
    bbb = 100*smoothed(np.load('Avs1_transfEqual_0%1s.npy'%(FigNum))/np.load('Avs1_transfNum_0%1s.npy'%(FigNum)))
